...
class rating_contract : public eosio::contract {
    ...
    private:
        double calculate_rating(const std::vector<double>&, const std::vector<double>&);
...
double rating_contract::calculate_rating(const std::vector<double>& y, const std::vector<double>& z)
{
    double my = calc_mean(y);
    double vary = calc_var(y, my);
    eosio_assert(vary != 0, "Math error");
...
void rating_contract::calculate(
    ...
    if (activities.size() > 0) {
        ...
        const int rounds_count = 3;
        bool modify_state = false;
        std::vector<double> randoms(activities.size());
        ...
        for (int i = 0; i < rounds_count; i++) {
            ...
            score += iround(calculate_rating(y, randoms) * 100);
        }
